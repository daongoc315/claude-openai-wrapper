...
Please report suspected vulnerabilities **privately** using [GitHub Security Advisories](https://github.com/daongoc315/claude-openai-wrapper/security/advisories/new).
...
- Constrain allowed paths with `CLAUDE_WRAPPER_ALLOWED_WORKING_DIR_PREFIXES`.
- Do not expose the service to the public internet.
- Set `CLAUDE_WRAPPER_API_KEY` if anything besides your own local process can connect.
- Treat enabled tools as local code/file access with the same OS permissions as the wrapper process.
- `bypassPermissions` is rejected unless `CLAUDE_WRAPPER_ALLOW_BYPASS_PERMISSIONS=1`; do not enable it outside fully trusted local environments.
